...
#DF <- read.table("AR.csv", sep=",", quote='"', row.names= c("环境 (%)", "耐久 (%)", "性能 (%)", "四驱高温 (%)"),  header=TRUE)
#print(DF )

DF <- read.table("bullet_AR.csv", sep=",",  header=TRUE)
DF$measure  <-  c("环境 (%)", "耐久 (%)", "性能 (%)", "四驱高温 (%)")
print(DF)
DF.bg <- bullet.graph(DF)

ggsave("output/bullet_AR.png", DF.bg, width=6, height=2.5)
