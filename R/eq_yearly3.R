...
all_yearly <- function(DF){
    ...
    plt <- ggplot(data = DF1,  aes(x=month,y = value  , color=variable)) +
        ...
        #ylim(c(0,150)) +
            ...
            scale_color_manual(values=c("#2A9836", "#39CCCC","#BCA409","#FF4136"), labels=c("重鼓","耐久","环境","高温"),
                        guide = guide_legend(title = "试验室"))  +
                        
                        
           theme(axis.text.x = element_text(size=10,   color="#BBBFC2"), #angle=90, vjust=0.1, 
               ...
               axis.title.x=element_blank(),
                    ...
                    legend.key = element_rect(color='#1F1F1F', fill = "#1F1F1F"),
    ...
    ggsave(file="output/yearly_201412_3.png", width=7, height=3)
