...
all_monthly <- function(d){
    ...
    plt <- ggplot(data = d, mapping=aes(x=a,y=d)) +
      ...
      #  geom_bar(stat="identity", fill="white", color="blue", position="dodge") +
        # blue
        geom_bar( mapping=aes(x=a,y=b), stat="identity", fill="white", alpha=I(.9), color="#0074D9") +
        # green
        geom_bar( mapping=aes(x=a,y=d), stat="identity", fill="green", alpha=I(.3), color="#2ECC40") +
        xlab("试验室") +
        ...
        #ylim(c(80,100)) +
        ggtitle(iconv("2014年12月设备工时","UTF-8","UTF-8")) + #iconv
        
        theme()
        
    #print(summary(diamonds))
    #ggplot(diamonds, aes(clarity, fill=cut)) + geom_bar(position="dodge")
    ggsave(file="monthly_workload_201412.png", width=6, height=3)

}

all_monthly_data <- data.frame(a=c(1:7),  b =130-4*c(1:7), d=7*c(11:17),
                                                e=c("里程\n耐久1#","里程\n耐久2#","里程\n耐久3#","环境1","环境2","环境3","结构\n耐久1"))
